...
namespace CompService.Presenters
{
    class MainPresenter
    {
        ...
        internal void CheckOutLoad(int selectedId)
        {
            try
            {
                ...
                if (checkOrder.CompletionDate == null)
                    view.CheckCompletionDate = Convert.ToString(DateTime.Now.ToShortDateString());
                else
        ...
        internal void HideIfNull()
        {
            try
            {
                if ((view.PartsInOrderData as object[][])[0][0] == null && (view.ServicesInOrderData as object[][])[0][0] == null)
                {
                    view.IfNullVisibility = false;
                    view.CheckCompletionDate = " - - ";
                }
                else
                    view.IfNullVisibility = true;
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.Message, "Ошибка!", MessageBoxButtons.OK, MessageBoxIcon.Error);
